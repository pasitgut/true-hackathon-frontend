...
const MenuBar = () => {
  ...
  return (
    <nav className="flex justify-center items-center p-4 md:p-6 bg-white min-h-[80px] md:min-h-[100px]">
      {/* <div className="flex flex-nowrap md:flex-nowrap items-center justify-center bg-[#F5F5F5] border-2 border-[#F5F5F5] rounded-[20px] px-6 md:px-10 py-6 md:py-8 shadow-md hover:shadow-lg transition-all duration-300 gap-4 md:gap-8 lg:gap-10 max-w-4xl"> */}
      <div className="flex flex-nowrap items-center justify-center bg-[#F5F5F5] border-2 border-[#F5F5F5] rounded-[20px] px-6 md:px-10 py-6 md:py-8 shadow-md hover:shadow-lg transition-all duration-300 gap-4 md:gap-8 lg:gap-10 max-w-4xl overflow-x-auto">
        {menuItems.map((item, index) => (
          <div
            key={index}
            className={`
              relative cursor-pointer transition-all duration-300 ease-in-out px-3 md:px-6 py-2 md:py-3 rounded-xl select-none text-center whitespace-nowrap
              ${activeItem === item.name 
                ? 'text-red-500 text-lg md:text-xl font-bold transform scale-105 md:scale-110' 
                : hoveredItem === item.name
                ? 'text-gray-700 text-base md:text-lg font-semibold transform scale-105 bg-gray-50'
                : 'text-gray-600 text-sm md:text-base font-medium hover:text-gray-800'
              }
            `}
            onClick={() => handleClick(item)}
            onMouseEnter={() => handleMouseEnter(item)}
